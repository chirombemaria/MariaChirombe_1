...
/* Counter
/*-----------------------------------------------------------------------------------*/
add_shortcode('risusagency_counter', 'risusagency_counter_function');

	function risusagency_counter_function( $atts, $content=null ) {
		extract(shortcode_atts(array(
			'number' => '3541',
			'title'  => '',
			'color'  => '#666666'
...
// Icon Box Heading
add_shortcode('risusagency_iconbox', 'iconbox_func');
...
function iconbox_func($atts, $content = null)
{
	...
	extract(shortcode_atts(array(
		...
		if($contentcolor != ''){
			$content_css = 'color: '.$contentcolor.';';
		} else {
			$content_css = '';
		}
		
		if($buttonbgcolor != ''){
			$button_css = 'color: '.$buttontcolor.';';
		} else {
			$button_css = '';
		}
		
		if($buttonbgcolor != ''){
			$buttonbg_css = 'color: '.$buttonbgcolor.';';
		} else {
			$buttonbg_css = '';
		}
		
		if($titlecolor != ''){
			$titlecss = 'color: '.$titlecolor.';';
		} else {
			$titlecss = '';
		}
		...
		if($style == '1') {
			...
			<div class="tl-spoiler pad0 contact iconbox '.esc_attr($class).' iconbox-style-' .esc_attr($style).' icon-color-'.esc_attr($iconcolor).' color-'.esc_attr($textcolor).'">
                        <div class="ok-icon" style="'.$titlecss.'"> '.$symbol.' </div>
                         <div class="su-clearfix tl-spoiler-content" style="'.$content_css.'"><a href="'.$url.'"><h6 style="'.$titlecss.'"> '. esc_html($title) .'</h6></a>'. do_shortcode($content) . '</div>
                 <a class="btns" style="'.$button_css.' '.$buttonbg_css.';" href="'.$url.'">' .$buttontext. '</a></div>';
		}
		elseif($style == '2') {
			$out = '
		...
		elseif($style == '3') {
			$out = '<div class="col-xs-12 col-sm-12 col-md-12 feature choose-section ' .esc_attr($class). ' iconbox-style-'.esc_attr($style).' icon-color-'.esc_attr($iconcolor).' color-'.esc_attr($textcolor).'">
				...
				<div class="item project-posts single-project-item mix <?php echo esc_attr($cate_slug); ?>">
                    ...
                    <img src="<?php echo esc_url($image);?>" alt=""></div>
						...
						<div class="hover-box">
							...
							<span><?php echo risusagency_excerpt(); ?></span>
						</div>
						</div>
							</div>
						</div>
...
?>
	...
	<div id="mc_embed_signup">
		<form id="mc-embedded-subscribe-form" class="newsletters-1" action="<?php echo esc_url( home_url('/') ); ?>wp-content/plugins/newsletter/do/subscribe.php" onsubmit="return newsletter_check(this)">
			<input name="ne" class="newsletter-email" type="email" data-icon="icon-envelope2" placeholder="<?php _e('Enter your email address', 'risusagency'); ?>">
...
<!-- 									<p class="author"><span>by </span> <?php the_author(); ?> </p> -->
  ...
  <p><?php echo $trimmed_content; ?></p>
									<a class="btn readmore" href="<?php the_permalink(); ?>"><?php _e('Read more', 'risusagency') ?></a>
								</div>
							</div>
							</article>
...
//Blog List
add_shortcode('blog_list', 'blogslist_func');


function blogslist_func($atts, $content = null)
{
...
<?php if
  ...
  <p><?php $days = round((date('U') - get_the_time('U')) / (60*60*24));
	if ($days==0) {
  ...
  <li> <a class="news_read" href="<?php echo esc_url($link); ?>">Read All News</a> </li>
        </ul>
      </div>
...
?>
          ...
          <div class="items bx-clone">
           		<?php $original_image = wp_get_attachment_url(get_post_thumbnail_id());
                    ...
                    <img src="<?php echo esc_url($image);?>" alt="">
           		<h2><a href="<?php the_permalink(); ?>" class=""><?php the_title(); ?></a></h2>
							<p><?php echo risusagency_excerpt(); ?></p>
							<a class="readmore" href="<?php the_permalink(); ?>"><?php _e('Read more', 'risusagency') ?></a>
          </div>
		<?php $i++; endwhile;?>
		<?php wp_reset_postdata(); ?>
	 	<?php endif; ?>
...
/* Imagebox 
/*-----------------------------------------------------------------------------------*/
if (!function_exists('risusagency_imagebox_function')) {
	function risusagency_imagebox_function( $atts, $content = null) {
		extract( shortcode_atts( array(
			'img' => '',
			'img_title' => '',
			'url' => '',
	...
	add_shortcode('risusagency_imagebox', 'risusagency_imagebox_function');
}
...
/* divider
/*-----------------------------------------------------------------------------------*/
if (!function_exists('risusagency_divider_function')) {
	function risusagency_divider_function( $atts, $content = null) {
		extract( shortcode_atts( array(
			'style' => '1',
			'icon' => '',
		...
		if($style == '8' && $icon != '') {
			$return2 = '<div class="risusagency-icon"><i class="fa '.esc_attr($icon).'"></i></div>';
		} else{
			$return2 = "";
		}
		  
		return '<div class="risusagency risusagency' .esc_attr($style). '" ' .$return. '>' .$return2. '</div>';  
	}
	add_shortcode('risusagency_risusagency', 'risusagency_divider_function');
}

/*-----------------------------------------------------------------------------------*/
/* New divider
/*-----------------------------------------------------------------------------------*/
if (!function_exists('risusagency_newdivider_function')) {
	function risusagency_newdivider_function( $atts, $content = null) {
		extract( shortcode_atts( array(
			'style' => 'solid',
			'line_color' => '#999999',
			'width' => '100%',
	...
	add_shortcode('risusagency_newdivider', 'risusagency_newdivider_function');
}

/*-----------------------------------------------------------------------------------*/
/* Headline
/*-----------------------------------------------------------------------------------*/
if (!function_exists('risusagency_headline_function')) {
	function risusagency_headline_function( $atts, $content = null) {
		extract( shortcode_atts( array(
			'type'   => 'h1',
			'margin' => '0 0 20px 0',
	...
	add_shortcode('risusagency_headline', 'risusagency_headline_function');
}
// Google Map
